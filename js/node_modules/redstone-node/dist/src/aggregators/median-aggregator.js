...
var medianAggregator = {
    getAggregatedValue: function (price, maxPriceDeviationPercent) {
        ...
        if (validValues.length === 0) {
            throw new Error("No valid values for symbol: ".concat(price.symbol));
        }
        ...
        for (var _i = 0, _a = Object.keys(price.source); _i < _a.length; _i++) {
            ...
            if (isNaN(value)) {
                ...
                if (value !== "error") {
                    logger.warn("Incorrect price value (NaN) for source: ".concat(sourceName, ". ")
                        + "Symbol: ".concat(symbol, ". Value: ").concat(value), price);
                }
            }
            else if (value <= 0) {
                logger.warn("Incorrect price value (<= 0) for source: ".concat(sourceName, ". ")
                    + "Symbol: ".concat(symbol, ". Value: ").concat(value), price);
            }
            else if (deviation > maxPriceDeviationPercent) {
                logger.info("Value ".concat(value, " has too big deviation for symbol: ").concat(symbol, " ")
                    + "and source: ".concat(sourceName, ". Deviation: (").concat(deviation, ")%. ")
                    + "Skipping...", price);
            }
            else {
        ...
        if (stableValues.length === 0) {
            throw new Error("All values have too big deviation for symbol: ".concat(price.symbol));
        }
