...
const log = log_1.Debug('Contracts');
...
class Contracts {
     ...
     */
    calculateDataUnionMainnetAddress(dataUnionName, deployerAddress, templateMainnetAddress) {
        ...
        const codeHash = keccak256_1.keccak256(`0x3d602d80600a3d3981f3363d3d373d3d3d363d73${templateAddress.slice(2)}5af43d82803e903d91602b57fd5bf3`);
        const salt = keccak256_1.keccak256(abi_1.defaultAbiCoder.encode(['string', 'address'], [dataUnionName, deployerAddress]));
        return address_1.getCreate2Address(this.factoryMainnetAddress, salt, codeHash);
    }
    /**
     * NOTE: if template address is not given, calculation only works for the newest currently deployed factory,
     *       i.e. can be used for "future deployments" but not necessarily old deployments
     * This can be used when deploying, but not for getDataUnion
     */
    calculateDataUnionSidechainAddress(mainnetAddress, templateSidechainAddress) {
        ...
        return address_1.getCreate2Address(this.factorySidechainAddress, bytes_1.hexZeroPad(mainnetAddress, 32), codeHash);
    }
    getMainnetContractReadOnly(contractAddress) {
        validateAddress('contractAddress', contractAddress);
        const provider = this.ethereum.getMainnetProvider();
        return new contracts_1.Contract(contractAddress, abi_2.dataUnionMainnetABI, provider);
    }
    getMainnetContract(contractAddress) {
    ...
    async getSidechainContract(contractAddress) {
        ...
        const duSidechain = new contracts_1.Contract(duSidechainAddress, abi_2.dataUnionSidechainABI, signer);
        return duSidechain;
    }
    async getSidechainContractReadOnly(contractAddress) {
        ...
        const duMainnet = this.getMainnetContractReadOnly(contractAddress);
        const duSidechainAddress = await duMainnet.sidechainAddress();
        const duSidechain = new contracts_1.Contract(duSidechainAddress, abi_2.dataUnionSidechainABI, provider);
        return duSidechain;
    }
    ...
    async getSidechainAmb() {
        if (!this.cachedSidechainAmb) {
            const getAmbPromise = async () => {
                ...
                return new contracts_1.Contract(sidechainAmbAddress, abi_2.sidechainAmbABI, sidechainProvider);
            };
    ...
    async getMainnetAmb() {
        ...
        const factoryMainnet = new contracts_1.Contract(this.factoryMainnetAddress, abi_2.factoryMainnetABI, mainnetProvider);
        const mainnetAmbAddress = await factoryMainnet.amb();
        return new contracts_1.Contract(mainnetAmbAddress, abi_2.mainnetAmbABI, mainnetProvider);
    }
    async getBinanceAdapter() {
        return new contracts_1.Contract(this.binanceAdapterAddress, abi_2.binanceAdapterABI, await this.ethereum.getSidechainSigner());
    }
    getBinanceAdapterReadOnly() {
        return new contracts_1.Contract(this.binanceAdapterAddress, abi_2.binanceAdapterABI, this.ethereum.getSidechainProvider());
    }
    async getBinanceSmartChainAmb(binanceSenderPrivateKey) {
        ...
        return new contracts_1.Contract(this.binanceSmartChainAMBAddress, abi_2.mainnetAmbABI, signer);
    }
    ...
    async transportSignaturesForMessage(messageHash, ethersOptions = {}) {
        ...
        try {
            ...
            const signers = signatures.map((signature) => wallet_1.verifyMessage(bytes_1.arrayify(message), signature));
    ...
    async deployDataUnion({ ownerAddress, agentAddressList, duName, deployerAddress, adminFeeBN, sidechainRetryTimeoutMs, sidechainPollingIntervalMs, confirmations, gasPrice }) {
        const mainnetProvider = this.ethereum.getMainnetProvider();
        ...
        const factoryMainnet = new contracts_1.Contract(this.factoryMainnetAddress, abi_2.factoryMainnetABI, mainnetWallet);
        const ethersOptions = {};
        if (gasPrice) {
            ethersOptions.gasPrice = gasPrice;
        }
        const duFeeFraction = units_1.parseEther('0'); // TODO: decide what the default values should be
        const duBeneficiary = '0x0000000000000000000000000000000000000000'; // TODO: decide what the default values should be
        const tx = await factoryMainnet.deployNewDataUnion(ownerAddress, adminFeeBN, duFeeFraction, duBeneficiary, agentAddressList, duName, ethersOptions);
        ...
        await utils_1.until(async () => await sidechainProvider.getCode(duSidechainAddress) !== '0x', sidechainRetryTimeoutMs, sidechainPollingIntervalMs);
        const dataUnion = new contracts_1.Contract(duMainnetAddress, abi_2.dataUnionMainnetABI, mainnetWallet);
        // @ts-expect-error
        dataUnion.sidechain = new contracts_1.Contract(duSidechainAddress, abi_2.dataUnionSidechainABI, sidechainProvider);
        return dataUnion;
    }
}
