...
class DataUnion {
    ...
    async getStats() {
        ...
        try {
            const [[totalRevenue, totalEarnings, totalAdminFees, totalDataUnionFees, totalWithdrawn, activeMemberCount, inactiveMemberCount, lifetimeMemberEarnings, joinPartAgentCount]] = abi_1.defaultAbiCoder.decode(['uint256[9]'], getStatsResponse);
            return {
                totalRevenue,
                totalAdminFees,
                totalDataUnionFees,
            ...
            const [[totalEarnings, totalEarningsWithdrawn, activeMemberCount, inactiveMemberCount, lifetimeMemberEarnings, joinPartAgentCount]] = abi_1.defaultAbiCoder.decode(['uint256[6]'], getStatsResponse);
            return {
                totalEarnings,
     ...
     */
    async getAdminFee() {
        let adminFeeBN = bignumber_1.BigNumber.from(0);
        try {
     ...
     */
    static async getVersion(ethereumProvider, contractAddress) {
    ...
    async setAdminFee(newFeeFraction, ethersOptions = {}) {
        ...
        try {
            ...
            const duSidechain = await this.getContracts().getSidechainContract(this.contractAddress);
            tx = await duSidechain.setFees(adminFeeBN, bignumber_1.BigNumber.from(0), ethersOptions); // DU DAO fee is zero for now
        }
        return waitForTx(tx);
    }
    /**
    ...
    async transferToMemberInContract(memberAddress, amountTokenWei) {
        ...
        const token = new contracts_1.Contract(tokenSidechainAddress, abi_2.erc20AllowanceAbi, signer);
